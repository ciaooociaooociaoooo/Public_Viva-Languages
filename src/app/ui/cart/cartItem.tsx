...
const CartItem = ({
  ...
  useEffect(() => {
    if (valueNumBar || valueNumBar === '') {
      if (initial.current === false) {
        // (To prevent triggering this useEffect when setting valueNumBar back to its original value (setValueNumBar(quantity)) when error occurs on updateSignedInCartQuantity or updateGuestCartQuantity.)
  ...
  return (
    <div
      ref={ref}
      className={clsx(
        'relative overflow-hidden p-2 last:border-b-0',
        cartItemIdWithIssue
          ...
          {category_name === 'Event' && (
            <div className='flex h-7 w-60/100 flex-col'>
              <NumBar
                ...
                inputProps={{
                  className: `transition-transform origin-center w-full relative z-1002 h-full text-lg text-center bg-overall-bg [&:hover:not(:focus):not(:disabled)]:shadow-[0_0_3px_var(--color-focus)] duration-100 ease-in`,
                  id: `cart-item-number-input-${program_id}`,
                  name: 'cart-item-number-input',
                  onBlur: () => handleBlur(),
                }}
                btnProps={{
                  className: `transition-transform origin-center w-10 relative z-1003 h-full bg-dusty-rose/30 flex items-center justify-center btn-hover-scale-glow btn-no-hover-scale-glow duration-100 ease-in`,
                }}
                plusBtnProps={{
              ...
              <div className='flex justify-end'>
                <label
                  ...
                  htmlFor={`cart-item-number-input-${program_id}`}
                >
                  attendees
                </label>
              </div>
