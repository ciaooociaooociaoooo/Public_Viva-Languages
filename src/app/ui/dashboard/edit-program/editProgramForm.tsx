...
import { deleteImageInCloudinary, editProgram } from '@/app/lib/actions'
...
export default function EditProgramForm({
  ...
  const rawDescContentRef = useRef<string>(program.description)
  ...
  const [price, setPrice] = useState<number | ''>(Number(program.price))

  const imageWidthRef = useRef<number | null>(program.image_width)

  const imageHeightRef = useRef<number | null>(program.image_height)

  const imageBytesRef = useRef<number | null>(program.image_size)

  const [imageURL, setImageURL] = useState<string>(program.image_url)

  const imagePublicIDRef = useRef<string>(program.image_public_id)

  const originalImagePublicIDRef = useRef<string>(program.image_public_id)
  ...
  const handleCancelClick = () => {
    ...
    // (If image is not updated - don't do anything to image.
    // If the uploaded image is not the original image - delete it on Cloudinary.)
    if (imageURL !== program.image_url && imagePublicIDRef.current) {
  ...
  const actionReducer = async (
    ...
    return await editProgram(_prevState, formData)
  }
  const initialState: EditProgramForm_ActionState = {
    type: null,
    errors: null,
    message: null,
    success: null,
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.set('program_id', program.program_id)
  ...
  useEffect(() => {
    ...
    if (
      ...
      // (If the image is updated - delete the original image in Cloudinary.)
      if (
        imagePublicIDRef?.current &&
        originalImagePublicIDRef?.current &&
        imagePublicIDRef?.current !== originalImagePublicIDRef?.current
      ) {
        const deleteImageInCloudinaryWithId = deleteImageInCloudinary.bind(
          null,
          String(originalImagePublicIDRef.current),
        )
                  ...
                  <div>
                    ...
                    <div className='flex items-center justify-start gap-3'>
                      ...
                      <select
                        ...
                        // size={0}
                        required
                        defaultValue={program?.teachers?.[0]?.id}
                        // value={teacherState}
                        // onChange={(e) => setTeacherState(e.target.value)}
                  ...
                  <div>
                    ...
                    <div className='flex items-center justify-start gap-3'>
                      ...
                      <select
                        ...
                        // size={0}
                        required
                        defaultValue={program.language_name}
                ...
                <div>
                  ...
                  <div className='flex items-center justify-start gap-3'>
                    ...
                    <select
                      ...
                      // size={0}
                      required
                      defaultValue={program.hosts?.[0]?.id}
              ...
              <div>
                ...
                <div className='flex items-center justify-start gap-3'>
                  ...
                  <select
                    ...
                    // size={0}
                    required
                    defaultValue={program.level_code}
                    // value={levelState}
                  ...
                  <div className='w-full py-2'>
                    <NumBar
                      ...
                      inputProps={{
                        ...
                        // defaultValue: program.enrollment_limit,
              ...
              <div className='flex h-50 items-center justify-start gap-3 lg:h-90'>
                ...
                >
                  <ImageUploaderWrapper
                    ...
                    disabled={isLoading}
                    // (On updating image success:
                    // If the previous image is the original image - don't delete it on Cloudinary.
                    // If the previous image is not the original image (e.g. it's the second time updating image) - delete it on Cloudinary.)
      ...
      <div className='mt-20'>
        ...
        <div className='flex flex-col items-center gap-5 lg:flex-row lg:justify-end lg:gap-4'>
          ...
          <Button
            ...
            disabled={isLoading}
          >
            {isLoading ? <LoaderOnBtn size={19} /> : 'Edit Program'}
          </Button>
        </div>
      </div>
    </form>
