...
import { GUEST_CART_COOKIE_NAME } from '@/app/lib/definitions'
...
export async function GET(req: Request) {
  try {
    ...
    const guestCookie = cookieStore.get(GUEST_CART_COOKIE_NAME)
    ...
    const query = `
      ...
      WHERE o.payment_intent_id = $1
        AND o.guest_id = $2::uuid
      GROUP BY o.id;
    `

    const res = await sql.query(query, [paymentIntentId, guestCookie.value])
